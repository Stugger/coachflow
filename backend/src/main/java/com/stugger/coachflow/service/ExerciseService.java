...
import com.stugger.coachflow.repository.exercise.ExerciseMediaRepository;
...
public class ExerciseService {
    ...
    public ExerciseService(ExerciseRepository exerciseRepository, ExerciseMediaRepository exerciseMediaRepository, TrainerRepository trainerRepository) {
    ...
    public List<ExerciseMediaResponse> getExerciseMedia(Long exerciseId, Long trainerId) {
        getTrainerOrThrow(trainerId);
        Exercise exercise = getExerciseOrThrow(exerciseId);
        validateTrainerCanViewExercise(exercise, trainerId);

        return exerciseMediaRepository.findByExerciseIdOrderBySortOrderAscIdAsc(exerciseId).stream()
                .map(ExerciseMediaResponse::new)
                .toList();
    }

    public ExerciseMediaResponse addExerciseMedia(Long exerciseId, @Valid CreateExerciseMediaRequest request) {
        getTrainerOrThrow(request.trainerId());
        Exercise exercise = getExerciseOrThrow(exerciseId);
        validateTrainerCanModifyExercise(exercise, request.trainerId());
        ...
        media.setUrl(TextUtils.trimToEmpty(request.url()));
        media.setTitle(TextUtils.trimToNull(request.title()));
        media.setSortOrder(request.sortOrder() == null ? 0 : request.sortOrder());
        media.setCreatedAt(LocalDateTime.now());

        return new ExerciseMediaResponse(exerciseMediaRepository.save(media));
    }
    ...
    private void validateTrainerCanViewExercise(Exercise exercise, Long trainerId) {
